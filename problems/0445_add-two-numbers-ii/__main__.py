...
class Solution:
    ...
    def helper(self, len_gap: int, l1: Optional[ListNode], l2: Optional[ListNode]) -> Tuple[Optional[ListNode], int]:
        if not l1 or not l2:
            return None, False
        ...
        sum = (0 if len_gap < 0 else l1.val) + \
            (0 if len_gap > 0 else l2.val) + carry
        return ListNode(sum % 10, next), sum // 10


solution = Solution()
node = solution.addTwoNumbers(ListNode(7, ListNode(2, ListNode(
    4, ListNode(3)))), ListNode(5, ListNode(6, ListNode(4))))
print(node.val)
print(node.next.val)
